...
-- E tuşuna basıldığında oynatılacak animasyon
RegisterNetEvent("animation:kenevir")
AddEventHandler("animation:kenevir", function(playerPed)
    local src = source
    
    FreezeEntityPosition(playerPed, true)  -- Oyuncuyu dondur
    TaskStartScenarioInPlace(playerPed, "WORLD_HUMAN_DRUG_DEALER", 0, true)  -- Animasyonu oynat
    Citizen.Wait(5000)  -- 5 saniye bekle
    ClearPedTasks(playerPed)  -- Görevleri temizle (dondurmayı kaldır)
    FreezeEntityPosition(playerPed, false)  -- Dondurmayı kaldır
    
    -- 'weed_baggy' item'larını envanterden sil
